...
import unsplash.UnsplashClient;
import unsplash.UnsplashImage;
import unsplash.UnsplashImageSearch;
import unsplash.UnsplashQueryImage;
...
//  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
public void selectImageForBackgroundSearchButton_actionPerformed(ActionEvent e, String searchString) {
  try {
    imageSearch(searchString);
  } catch (IOException ioException) {
    ioException.printStackTrace();
  }
}

  public void imageSearch(String searchString) throws IOException {
    try{
      ...
      int fileCount = 1 ;
      for (int b=1; b<4 ; b++){
        UnsplashQueryImage images = client.getPhotos(searchString, Integer.toString(b));
        UnsplashImage[] array =images.getUnsplashImageArray();

        for(int imgCount=0; imgCount<10 ; imgCount++){
          UnsplashImage loopImage = array[imgCount];
          if(loopImage != null){
            //adding image owner details to the list
            String imageAutherName = loopImage.user.getImageOwnerName();
            userList.add(imageAutherName);

            URL url = new URL(loopImage.getImageUrl());
            InputStream in = new BufferedInputStream(url.openStream());
            OutputStream out = new BufferedOutputStream(new FileOutputStream("D:\\JWildFireFiles\\"+ (fileCount)+".jpg"));
        ...
        FileWriter writer = new FileWriter("D:\\JWildFireFiles\\imageAuthors.txt");
        int authorWriteCounter = 1;
        for(String str: userList) {
          writer.write( Integer.toString(authorWriteCounter) + " "+str + System.lineSeparator());
          authorWriteCounter++;
        }
        writer.close();
      }
  ...
  public void searchWordCount(ActionEvent e) {
    try {
      WordCoundFinder.findWordCount();
    } catch (IOException ioException) {
      ioException.printStackTrace();

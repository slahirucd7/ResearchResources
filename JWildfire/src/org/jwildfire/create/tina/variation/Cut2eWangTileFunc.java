...
public class Cut2eWangTileFunc  extends VariationFunc {
	...
	double tile(vec2 uv, int tile)
	{
	    ...
	    {
	        case 0: return 1.414;
	        case 1: return Math.max(tile0(uv), .15 - G.length(uv));
	        case 2: return tile0(new vec2(uv.y,uv.x));
	        case 3: return tile1(uv);
	        case 4: return tile0(new vec2(uv.x, -uv.y));
	        case 5: return tile2(uv);
	        case 6: return tile1(new vec2(uv.x, -uv.y));
	        case 7: return tile3(uv);
	        case 8: return tile0(new vec2(uv.y, -uv.x));
	        case 9: return tile1(new vec2(-uv.x, uv.y));
	        case 10: return tile2(new vec2(uv.y,uv.x));
	        case 11: return tile3(new vec2(uv.y,uv.x));
	        case 12: return tile1(new vec2(-uv.x, -uv.y));
	        case 13: return tile3(new vec2(-uv.x, uv.y));
	        case 14: return tile3(new vec2(-uv.y, uv.x));
	...
	double map(vec2 uv)
	{
	    ...
	    if(hash(id) >= .5)
	        b += 1;
	    if(hash(id.multiply(-1.)) >= .5)
	        b += 8;
	    if(hash(id.minus( new vec2(0., 1.))) >= .5)
	        b += 4;
	    if(hash((id.plus(new vec2(1., 0.)).multiply(-1.0))) >= .5)
	        b += 2;
	...
	double height(vec2 uv)
	{
	  ...
	  public void transform(FlameTransformationContext pContext, XForm pXForm, XYZPoint pAffineTP, XYZPoint pVarTP, double pAmount) {
		    ...
		    double color;
		    color= 1.0;
		    color = G.mix(1.0,.0, G.smoothstep(0., 2. /2000.0 , Math.min(v, Math.abs(v - .1) - .1) / zoom)) ;
		    color = G.sqrt(color);
